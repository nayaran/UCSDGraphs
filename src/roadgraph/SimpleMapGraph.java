...
public class SimpleMapGraph extends MapGraph {
	...
	protected List<GeographicPoint> findPathUsingBFS(MapNode startNode, MapNode endNode,
			Consumer<GeographicPoint> nodeSearched) {
	...
	private List<GeographicPoint> constructPathFromParentMap(MapNode startNode, MapNode endNode,
			...
			if (!visited.contains(currentNode)) {
				...
				for (MapNode neighbor : neighborDistanceMap.keySet()) {
					...
					if (!visited.contains(neighbor)) {
						...
						neighbor.setEstimatedDistanceToDestination(
								neighbor.getLocation().distance(endNode.getLocation()));
